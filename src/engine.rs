...
fn load_source_markdown(root: &Path, subdir: &str, kind: SourceKind) -> Result<Vec<Command>> {
    ...
    // Local source files override curated files.
...
fn load_singleton(
    ...
    // Use the last singleton overlay.
...
fn render_rules(
        ...
        (paths.nonroot_rule_dir.as_ref(), paths.root_rule.as_ref())
    {
        ...
        // Rootless formats emit each rule as a module.
        for rule in normal {
            if spec.name == "takt" {
...
fn render_commands(
    ...
    for command in &commands {
        ...
        if spec.name == "cline" {
            // Cline writes command bodies without frontmatter.
            fm.clear();
        }
    ...
    if has_blocking_gate {
        ...
        if has_prompt_gate {
            lines.extend([
                ...
                "    }".into(),
                "  } catch {".into(),
                "    // Write to stderr when UI notifications fail.".into(),
                "  }".into(),
...
fn render_codex_bash_rules(effective: &Map<String, Value>) -> String {
    let header = ["# Generated from .carabiner/permissions.jsonc permission.bash."];
...
fn delete_orphans(
    ...
    // The skills feature owns Devin and Warp command deletion.
